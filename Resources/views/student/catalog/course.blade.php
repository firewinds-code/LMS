    ...
    <div class="col-md-8">
        <div class="card course-info profile-widget mt-0">
            <div class="profile-widget-header">
                <div class="profile-widget-items">
                    ...
                    <div class="profile-widget-item">
                        ...
                        <div class="profile-widget-item-value">@php $countClass = $totalClasses != 0 ? $totalClasses : \App\Course::find($id)->lessons()->count() @endphp {{ $countClass }}</div>
                    </div>
                    <div class="profile-widget-item">
            ...
            <div class="card-footer text-center">
                {{-- @if($attendance < 100) --}}
                    <a  class="btn btn-primary mb-2  btn-lg" href="{{ route('student.course.intro', ['id'=>$id]) }}">{{  __('Go To - Class Lecture')  }}</a>
                   {{-- @else
                   <a  class="btn btn-success mb-2  btn-lg disbaled" href="javaScript::void()">{{  __('Classes Completed')  }}</a>
                @endif --}}
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col-md-8">
        <ul class="nav nav-pills" id="myTab3" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" id="home-tab3" data-toggle="tab" href="#home3" role="tab" aria-controls="home" aria-selected="true"><i class="fa fa-info-circle"></i> {{  __lang('Course Details')  }}</a>
            </li>
            <li class="nav-item">
        ...
        <div class="tab-content" id="myTabContent2">
            ...
            <div class="tab-pane fade" id="profile3" role="tabpanel" aria-labelledby="profile-tab3">
                ...
                @php  $sessionVenue= $row->venue; $i=0; @endphp

                @foreach($rowset as $row2)
                     @php $i++; @endphp
                <div class="card">
                    <div class="card-header"><h4>{{$i}}&sdot;&nbsp;{{  $row2->name }}</h4>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tr>
                            ...
                            <td>
                                @if(date('Y-m-d',strtotime($testRow->opening_date)) > date('Y-m-d'))
                                <span class="btn btn-success disabled">{{__('Link will Open on this')}}&nbsp; {{date('Y-m-d',strtotime($testRow->opening_date))}}</span>
                                @endif
                                @if( !$studentTest->hasTest($testRow->test_id,$studentId) && date('Y-m-d',strtotime($testRow->opening_date)) < date('Y-m-d'))
                                <span class="btn btn-danger disabled">{{__('Link expired on')}}&nbsp; {{date('Y-m-d',strtotime($testRow->opening_date))}}</span>
                                @endif
                                @if($attendance < 100)
                                <span class="btn btn-warning disabled">{{__('Class Lecture not be Completed')}}</span>
                                @endif
                                @if( (!$studentTest->hasTest($testRow->test_id,$studentId) || !empty($testRow->allow_multiple))  && (date('Y-m-d',strtotime($testRow->opening_date)) == date('Y-m-d') && date('Y-m-d',strtotime($testRow->closing_date) >= date('Y-m-d'))) &&($attendance == 100))
                                <a  target="_blank" href="{{  route('student.test.taketest',array('id'=>$testRow->test_id)) }}" class="btn btn-primary " >{{  __lang('take-test')  }}</a>
                                @endif
                                @if(($studentTest->hasTest($testRow->test_id,$studentId)) && (empty($testRow->allow_multiple)))
                                    <a  href="javaScript::void(0)" class="btn btn-danger disabled" >{{  __('already-taken') }}</a>
                                @endif
                            </td>
                        </tr>
                        @php  endif;  @endphp
