<div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
    ...
    <input class="form-control" name="name" type="text" id="name"
        value="{{ old('name', isset($admin->name) ? $admin->name : '') }}">
...
<div class="form-group {{ $errors->has('last_name') ? 'has-error' : '' }}">
    ...
    <input class="form-control" name="last_name" type="text" id="name"
        value="{{ old('last_name', isset($admin->last_name) ? $admin->last_name : '') }}">
        ...
        <select required name="role" id="roles" class="form-control select2">
            ...
            @foreach (\App\AdminRole::get() as $role)
                @if ($role->name == 'Super Administrator' || $role->name == 'Administrator')
                    @continue
                @endif
                <option @if (old('role', $admin->admin->admin_role_id) == $role->id) selected @endif value="{{ $role->id }}">
                    {{ $role->name }}</option>
            @endforeach
        </select>
    @else
        <select required name="role" id="roles" class="form-control select2">
            ...
            @foreach (\App\AdminRole::get() as $role)
                @if ($role->name == 'Super Administrator' || $role->name == 'Administrator')
                    @continue
                @endif
                <option @if (old('role') == $role->id) selected @endif value="{{ $role->id }}">
                    {{ $role->name }}</option>
            @endforeach
        </select>
    @endif
    ...
    <select name="status" class="form-control" id="status">
        @foreach (json_decode('{"1":"' . __('default.yes') . '","0":"' . __('default.no') . '"}', true) as $optionKey => $optionValue)
            <option value="{{ $optionKey }}"
                {{ null !== old('status', @$admin->status) && old('admin', @$admin->status) == $optionKey ? 'selected' : '' }}>
                {{ $optionValue }}</option>
        @endforeach
    </select>
...
<script type="text/javascript">
    ...
    function image_upload(field, thumb) {
        ...
        $('#content').prepend(
            '<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="{{ basePath() }}/admin/filemanager?&token=true&field=' +
        ...
        $('#dialog').dialog({
            ...
            close: function(event, ui) {
                if ($('#' + field).attr('value')) {
                    $.ajax({
                        url: '{{ basePath() }}/admin/filemanager/image?&image=' +
