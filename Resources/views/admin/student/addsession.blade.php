...
<div class="col-md-12 col-sm-12 col-xs-12">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form id="sessionform" class="form-horizontal" role="form" method="post" action="{{ adminUrl(array('controller'=>'student','action'=>$action.'session','id'=>$id,'type'=>$type)) }}" enctype="multipart/form-data">
            @csrf
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                 <label for="password1" class="control-label">{{ __lang('session-instructors') }} ({{ __lang('optional') }})</label>
                                     @isset($trainer)
                                     <select class='form-control select2', multiple='true' name="session_instructor_id[]">
                                     @foreach($trainer as $row)
                                       <option value="{{ $row->id }}">{{ $row->username }}({{ $row->email }})-({{ $row->role }})</option>
                                    @endforeach
                                    </select> 
                                     @endisset
                                </div>
