...
class StudentController extends Controller {
    ...
    public function __construct(ClassAcknowledge $classAck, StudentTable $student){
        $user= '';
        if(defined('USER_ID')){
            $user = '/'.USER_ID;
        }
    ...
    public function acknowlwdge(Request $request)
    {
        try{
            $condition = $this->classAck::where(function($query) use ($request) {
                $query->where('courses_id',$request->course_id)->where('lessons_id', $request->id)
               ->where('students_id', $this->getId());
            })->update(['is_acknowledge' => 1]);
          
            $sessionLessonTable = new SessionLessonTable();
            $rowset = $sessionLessonTable->getSessionRecords($request->course_id); 
                                   
        if($condition)
        {  
            return response()->json(['html'=>view('student.ajax.testclasses',compact('rowset'))->render(),'success'=>true, 'message' => 'Class Acknowledged Successfully !']); 
        } 
          return response()->json(['error'=>true, 'message' => 'Class Acknowledged failed !']);
        } catch (\Exception $ex) {
    ...
    public function timetable(Request $request,$id,$slug)
    {
        ...
        $studentCourse = false;
            ...
            if ($studentSessionTable->enrolled($studentId, $id)) {
                $studentCourse = $this->getStudent()->studentCourses()->where('course_id',$id)->where('student_id',$studentId)->first();
                $enrolled= true;
        ...
        'pageTitle'=>__lang('Session Details'),'table'=>$sessionLessonAccountTable,
        'id'=>$id,
        'studentId'=>$studentId,
        'studentSessionTable' => $studentSessionTable,
    ...
    public function classAckAndTest(Request $request)
    {
        ...
        return response()->json(['success' => true, 'htmltest'=>view('student.ajax.classtestack',compact('studentCourse','studentTest','questionTable','rowset','row','id','studentId','studentSessionTable','tests'))->render(),  'htmlclass'=>view('student.ajax.testclasses',compact('rowset','row','id','studentId','studentSessionTable'))->render(),  200,]);
    }
    ...
    public function downloadcertificate(Request $request,$id){
        ...
        //     $pdf->loadHTML($html)->setPaper('a4')->setOrientation($orientation)->setOption('margin-bottom', 0)->setOption('margin-top', 0)->setOption('margin-right', 0)->setOption('margin-left', 0)->setOption('page-width',162)->setOption('page-height',230)->setOption('disable-smart-shrinking',false);
