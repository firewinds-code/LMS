...
class SessionController extends Controller
{
    ...
    public function __construct(ClassAcknowledge $classAck)
      {
    ...
    public function editcourse(Request $request,$id){
     ...
     //   $sessionLessonTable = new SessionLessonTable();
        ...
        $output['id'] = $id;
        $output['type'] = $type;
        $output['session_end_date'] = Carbon::parse($course->end_date)->toDateString();
        $output['session_date'] =  Carbon::parse($course->start_date)->toDateString();;
        return view('admin.session.addcourse',$output);
    }
    ...
    public function setclass(Request $request,$id)
    {
        ...
        $datadd  = StudentCourse::where('course_id', $sessionId)->first();
        $data = ['courses_id' => $sessionId,
                'lessons_id' => $id,
                'students_id' => !empty($datadd) ? $datadd->student_id : 0,
                'created_at' => date('Y-m-d h:m:s'),
                'updated_at' => date('Y-m-d h:m:s')];
        $this->classAck->insert($data);
    ...
    public function deleteclass(Request $request,Lesson $lesson,Course $course){
     /*   $id = request()->get('id');
        ...
        $this->classAck::where('lessons_id', $lesson->id)->where('courses_id', $course->id)->delete();
        $sessionLessonTable= new SessionLessonTable();
    ...
    public function edittest(Request $request,$id){
        ...
        $output['closing_date'] = $data['closing_date'];
        $output['opening_date'] = $data['opening_date'];
