...
class StudentController extends Controller
{
    ...
    public function __construct(ClassAcknowledge $classAck,CourseLessonAdmin $courseLesson){
        $user= '';
        if(defined('USER_ID')){
            $user = '/'.USER_ID;
        }
 ...
 private function makeUploadDir(){
    ...
    public function active(Request $request)
    {
        ...
        $pageTitle = __('Active Trainee').': '.$table->getTotalActiveStudents();
    ...
    public function add(Request $request)
    {
        ...
        $output['form'] = $form;
        $output['pageTitle']= __('Add Trainee');
        $output['action'] = 'add';
        $output['id']=0;
    ...
    public function view(Request $request,$id)
    {
        ...
        return viewModel('admin',__CLASS__,__FUNCTION__,array(
            ...
            'pageTitle'=>__('Trainee Details').': '.$row->last_name.' '.$row->name,
            'validator'=>$validator,
            'attendanceTable'=>$attendanceTable,
            'id'=>$id
        ));
    ...
    public function edit(Request $request,$id)
    {
        ...
        $output['form'] = $form;
        $output['id'] = $id;
        $output['pageTitle']= __('Edit Trainee');
        $output['row']=$student;
        $output['action'] = 'edit';
        $output['id']=$id;
...
/*                $studentTable = new StudentTable();
    ...
    public function addsession(Request $request,$type){
        ...
        if($request->isMethod('post')){
            ...
            if($form->isValid()){
                ...
                if($request->hasFile('picture')){
                    ...
                    $newPath = $this->uploadDir.'/'.time().'_'.sanitize($file->getClientOriginalName());
                    $this->makeUploadDir();

                    $stldestinationPath = public_path('/tmp/cache/'.$this->uploadDir);
                    $file->move($stldestinationPath, $newPath);
        ...
        $trainer = DB::table('admins')->leftjoin('users', 'users.id','admins.user_id')
                                      ->leftjoin('admin_roles','admins.admin_role_id', 'admin_roles.id',['admin_role_name'=>'name'])->orderBy('users.name','asc')
                                      ->select('users.name as username','admin_roles.name as role','users.id as id','users.email as email')
                                      ->get();
    ...
    public function enroll(Request $request,$id)
    {
        ...
        $studentRow = $studentTable->getRecord($id);
            ...
            $text->setValue($filter);
            $attendanceTable = new AttendanceTable();
            $paginator = $studentTable->getStudents(true,$filter);
            $total = User::where('role_id',2)->whereHas('student')->count();
            $paginator->setCurrentPageNumber((int)$request->get('page', 1));
            $paginator->setItemCountPerPage(10);
            $pageTitle = 'Trainee : '.$total;
        ...
        if($request->isMethod('post'))
        {
            ...
            {
                return view('admin.student.index',compact('paginator','filter','text','pageTitle','attendanceTable','studentSessionTable'));
            }

            $this->classAck::where(function($query) use ($sessionId){
                $query->where('courses_id', $sessionId)->where('students_id', 0);
            })->update(['students_id' => $id]);

            $condition = $this->classAck::where(function($query) use ($sessionId,$id){
                $query->where('courses_id', $sessionId)->where('students_id', $id);
            })->doesntExist();
            if($condition)
            {
               $data = DB::select('SELECT lesson_id FROM course_lesson where course_id = '.$sessionId.'');
               foreach($data as $list)
               {
                $ackdata = ['students_id' => $id, 'lessons_id' => $list->lesson_id, 'courses_id' => $sessionId , 'is_acknowledge' => 0];
                $this->classAck::create($ackdata);
               }
            }
            $this->enrollStudent($id,$sessionId);
            session()->flash('flash_message',__lang('you-have-enrolled'));
            return view('admin.student.index',compact('paginator','filter','text','pageTitle','attendanceTable','studentSessionTable'));
        }
